...
# نموذج كشف عام (Object Detection) لتمييز نوع الهدف: بشر / حيوانات / دمى.
# نموذج Pose أعلاه يكتشف "الأشخاص" فقط، لذا نحتاج نموذجًا ثانيًا مختلفًا
# لتصنيف الفئات المتعددة (صندوق كامل + تسمية) بأسلوب ESP الألعاب.
DETECT_MODEL_PATH = os.environ.get("YOLO_DETECT_MODEL", "yolo11s.pt")
DETECT_INFERENCE_IMG_SIZE = 960

CONFIDENCE_THRESHOLD = 0.30

# فئات COCO المستهدفة ورمزها الظاهر على الصندوق + لون مميز لكل نوع (BGR)
# ملاحظة مهمة: لا توجد فئة "دمية/تمثال" حقيقية في قاعدة بيانات COCO التي
# تدرب عليها YOLO11 الافتراضي؛ أقرب فئة متاحة لها هي "teddy bear" (دبدوب
# محشو)، لذا نستخدمها كتقريب لعرض دمى/ألعاب محشوة فقط - وليست أي دمية.
TARGET_CLASSES = {
    0: ("HUMAN", (60, 220, 60)),        # أخضر
    14: ("BIRD", (255, 140, 0)),        # برتقالي مزرق
    15: ("CAT", (255, 140, 0)),
    16: ("DOG", (255, 140, 0)),
    17: ("HORSE", (255, 140, 0)),
    18: ("SHEEP", (255, 140, 0)),
    19: ("COW", (255, 140, 0)),
    20: ("ELEPHANT", (255, 140, 0)),
    21: ("BEAR", (255, 140, 0)),
    22: ("ZEBRA", (255, 140, 0)),
    23: ("GIRAFFE", (255, 140, 0)),
    77: ("DOLL", (200, 60, 220)),       # بنفسجي (تقريب: teddy bear)
}
...
logger.info("جاري تحميل نموذج الكشف العام (بشر/حيوانات/دمى) ... (%s)", DETECT_MODEL_PATH)
...
def run_general_detection(image_bgr: np.ndarray, imgsz: int):
    """كشف متعدد الفئات (بشر/حيوانات/دمى) بصندوق كامل وتسمية، عبر نموذج
    الكشف العام. يعيد قائمة (x1, y1, x2, y2, class_id, confidence)."""
...
def _draw_esp_box(img: np.ndarray, x1: int, y1: int, x2: int, y2: int, label: str, conf: float, color) -> None:
    """صندوق كامل احترافي بأسلوب كشف الأهداف بالألعاب: إطار + تسمية بالنوع والثقة%."""
    ...
    cv2.rectangle(img, (x1, ty1), (x1 + tw + pad * 2, y1), color, -1)
    cv2.putText(img, text, (x1 + pad, y1 - pad), font, scale, (10, 10, 10), thick, cv2.LINE_AA)


def draw_esp_overlay(
    image_bgr: np.ndarray,
    detections,
    pose_result,
) -> Tuple[np.ndarray, dict]:
    """
    يرسم صناديق ESP كاملة لكل كائن مكتشف (بشر/حيوانات/دمى) بلون مميز
    لكل نوع + تسمية ونسبة ثقة، ويضيف هيكلًا عظميًا متوهجًا فوق الأشخاص
    الواضحين فقط (باستخدام نتائج نموذج Pose المنفصل، إن وُجدت).
    يعيد الصورة المعدّلة + قاموسًا بعدد كل نوع مكتشف.
    """
    ...
    for (x1, y1, x2, y2, cls_id, conf) in detections:
        label, color = TARGET_CLASSES.get(cls_id, ("OBJECT", (200, 200, 200)))
        counts[label] = counts.get(label, 0) + 1
        _draw_esp_box(output, x1, y1, x2, y2, label, conf, color)

    # الهيكل العظمي المتوهج (إن توفرت نتائج Pose) - للأشخاص الواضحين فقط
    if pose_result is not None and pose_result.boxes is not None and pose_result.keypoints is not None:
        ...
        for idx in range(person_count):
            ...
            if box_area_ratio < MIN_PERSON_AREA_RATIO_FOR_SKELETON:
                continue
...
def process_frame(image_bgr: np.ndarray, *, for_video: bool = False) -> Tuple[np.ndarray, dict]:
    """
    يجمع بين نموذج الكشف العام (بشر/حيوانات/دمى بصندوق كامل) ونموذج Pose
    (هيكل عظمي للأشخاص). للفيديو نكتفي بنموذج الكشف العام فقط (بدون
    هيكل عظمي) لتفادي إبطاء المعالجة بتشغيل نموذجين على كل إطار.
    """
    detect_imgsz = VIDEO_INFERENCE_IMG_SIZE if for_video else DETECT_INFERENCE_IMG_SIZE
    detections = run_general_detection(image_bgr, detect_imgsz)

    pose_result = None
    if not for_video:
        pose_result = run_pose_detection(photo_model, image_bgr, PHOTO_INFERENCE_IMG_SIZE)

    annotated, counts = draw_esp_overlay(image_bgr, detections, pose_result)
    annotated = apply_watermark(annotated)
    return annotated, counts


def build_caption(counts: dict) -> str:
    if counts:
        lines = "\n".join(f"• {label}: *{n}*" for label, n in sorted(counts.items()))
    else:
        lines = "• No targets detected"
    return (
        "✅ *Detection Complete*\n\n"
        f"{lines}\n\n"
        "🧠 Model: YOLO11 (Detection + Pose)\n"
        "📡 ESP overlay applied\n\n"
...
def _process_video_sync(tmp_in: str, tmp_out_raw: str, progress_cb) -> int:
    ...
    while True:
        ...
        if frame_idx % VIDEO_FRAME_SKIP == 0 or last_annotated is None:
            annotated, counts = process_frame(frame, for_video=True)
            last_annotated = annotated
            for label, n in counts.items():
                max_counts[label] = max(max_counts.get(label, 0), n)
        else:
